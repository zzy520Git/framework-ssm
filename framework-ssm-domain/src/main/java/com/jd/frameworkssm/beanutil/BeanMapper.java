 ...
 *******************************************************************************/
package com.jd.frameworkssm.beanutil;
...
public class BeanMapper {
     ...
     */
    public <S, D> D map(S source, Type<S> sourceType, Type<D> destinationType) {
     ...
     * 预先通过BeanMapper.getType() 静态获取并缓存Type类型，在此处传入
     */
    public <S, D> List<D> mapList(Iterable<S> sourceList, Type<S> sourceType, Type<D> destinationType) {
     ...
     */
    public <S, D> D[] mapArray(final D[] destination, final S[] source, final Class<D> destinationClass) {
     ...
     * 预先通过BeanMapper.getType() 静态获取并缓存Type类型，在此处传入
     */
    public <S, D> D[] mapArray(D[] destination, S[] source, Type<S> sourceType, Type<D> destinationType) {
     ...
     */
    public <E> Type<E> getType(final Class<E> rawType) {
